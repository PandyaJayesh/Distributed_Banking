...
def acquire_lock(account_number):
    ...
    with open(f"queue1_files{account_number}.queue", "a") as queue_file:
        queue_file.write("{}\n".format(os.getpid()))

    # Wait until it's this instance's turn to proceed
    while True:
        with open(f"queue1_files{account_number}.queue","r+") as queue_file:
...
def update_balance(account_number, new_balance, deposit=0, withdraw=0):
  if random_number<=0.9:
    if account_number in shared_accounts:
      ...
      server1_file = f"./servers/server1/data_{account_number}"
      os.makedirs(os.path.dirname(server1_file), exist_ok=True)
      acquire_lock(account_number)
      with open(server1_file, 'a+') as f:
        if deposit==0:
...
def initialize_accounts():
  ...
  try:
     with open("./servers/server1/data_101","r") as f1, open("./servers/server1/data_102","r") as f2, open("./servers/server1/data_103","r") as f3, open("./servers/server1/data_104","r") as f4, open("./servers/server1/data_105","r") as f5:
        try:
          read1=f1.readlines()
   ...
   while True:
    try:
      for account_number in shared_accounts:
          acquire_lock(account_number)
          with open(f"./servers/server1/data_{account_number}","r+") as f2, open(f"./servers/master_server/data_{account_number}","r") as f1:
            try:
              read1=f1.readlines()
              read2=f2.readlines()
              compare1=read1[len(read1)-1].split(":")
...
def deposit(account_number, amount):
  if random_number<=0.9:
    if account_number in shared_accounts:
      ...
      server2_proxy = xmlrpc.client.ServerProxy("http://localhost:8001/")
      server2_proxy.update_balance(account_number,shared_accounts[account_number],amount,0)
      counter[account_number] += 1
    ...
    if random_number<=0.9:
      if account_number in shared_accounts:
          if shared_accounts[account_number] >= amount:
              ...
              server2_proxy = xmlrpc.client.ServerProxy("http://localhost:8001/")
              server2_proxy.update_balance(account_number,
    ...
    if random_number<=0.9:
      if account_number_from in shared_accounts and account_number_to in shared_accounts:
          if shared_accounts[account_number_from] >= amount:
              ...
              server2_proxy = xmlrpc.client.ServerProxy("http://localhost:8001/")
              server2_proxy.update_balance(account_number_from,
                                            shared_accounts[account_number_from] ,0,amount)
              server2_proxy.update_balance(account_number_to,
              ...
              server1_file = f"./servers/server1/data_{account_number_from}"
              os.makedirs(os.path.dirname(server1_file), exist_ok=True)
              ...
              server1_file = f"./servers/server1/data_{account_number_to}"
              os.makedirs(os.path.dirname(server1_file), exist_ok=True)
              ...
              return "Transfer of {} from account {} to account {} successful. New balance of account {}: {}".format(amount, account_number_from, account_number_to, account_number_from, shared_accounts[account_number_from])
          else:
...
# Create a simple XML-RPC server for Server 1
server = SimpleXMLRPCServer(("localhost", 8000))
print("Server 1 listening on port 8000...")
